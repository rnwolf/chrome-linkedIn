...
function downloadSavedPosts() {
  ...
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    if (tabs[0].url.includes('linkedin.com/my-items/saved-posts')) {
      ...
      extractSavedPosts(tabs[0].id, postsCount, downloadImages, statusElement);
    } else {
      ...
      chrome.tabs.create({url: 'https://www.linkedin.com/my-items/saved-posts/'}, function(tab) {
        ...
        setTimeout(() => {
          extractSavedPosts(tab.id, postsCount, downloadImages, statusElement);
...
function extractSavedPosts(tabId, postsCount, downloadImages, statusElement) {
  ...
  chrome.scripting.executeScript({
    target: {tabId: tabId},
    args: [postsCount],
    function: extractLinkedInSavedPosts
  }, (results) => {
    if (results && results[0].result) {
      const posts = results[0].result;

      if (posts.length === 0) {
        statusElement.innerHTML = 'No saved posts found.';
        return;
      }
      ...
      if (downloadImages) {
          ...
          if (post.images && post.images.length > 0) {
            post.images.forEach((image, imgIndex) => {
              if (image.src) {
                ...
                const author = post.author.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20);
                const filename = `${timestamp}_${author}_post${postIndex+1}_image${imgIndex+1}.jpg`;

                const imagePromise = downloadImage(image.src, filename);
                downloadPromises.push(imagePromise);
                imageCount++;

                // Add local reference to the image
                image.localPath = `linkedin_images/${filename}`;
              }
            });
          }
        });
        ...
        chrome.downloads.download({
          url: jsonUrl,
          filename: `linkedin_saved_posts_${timestamp}.json`,
          saveAs: false
        }, () => {
      ...
      statusElement.innerHTML = 'Error extracting posts.';
    }
  });
}});
      ...
      Promise.allSettled(downloadPromises).then(() => {
        statusElement.innerHTML = `Downloaded ${posts.length} posts with ${imageCount} images successfully!`;
      }).catch(err => {
        console.error('Error downloading images:', err);
        statusElement.innerHTML = `Downloaded ${posts.length} posts, but some images may have failed.`;
      });
    } else {
      statusElement.innerHTML = 'Error extracting posts.';
    }
...
function downloadImage(imageUrl, filename) {
  return new Promise((resolve, reject) => {
    ...
    chrome.downloads.download({
      url: imageUrl,
      filename: `linkedin_images/${filename}`,
      saveAs: false
    }, (downloadId) => {
